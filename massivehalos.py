...
url = "http://www.illustris-project.org/api/Illustris-2/snapshots/135/subhalos/" + search_query
...
while n < len(c):
	...
	half_star_mass_radius = sub['halfmassrad_stars']
	half_gas_mass_radius =  sub['halfmassrad_gas']
	half_dm_mass_radius = sub['halfmassrad_dm']
	gas_radii_ratio1 =    half_gas_mass_radius /  half_dm_mass_radius
	star_radii_ratio1 = half_star_mass_radius / half_dm_mass_radius
	
	dm_mass_at_half = sub['massinhalfrad_dm']
	gas_mass_at_half =  sub['massinhalfrad_gas']
	star_mass_at_half =  sub['massinhalfrad_stars']
	star_mass = sub['mass_stars']
	gas_mass = sub['mass_gas']
	dm_mass_tot = sub['mass_dm']	
	average_dark_density_at_half =  (dm_mass_at_half) / ((half_dm_mass_radius)**3)
	average_dark_density_at_half = average_dark_density_at_half * 4.18879
	average_star_density_at_half = (star_mass_at_half)  / ((half_star_mass_radius)**3)
	average_star_density_at_half = average_star_density_at_half * 4.18879
	average_gas_density_at_half = (gas_mass_at_half)  / ((half_gas_mass_radius)**3)
	average_gas_density_at_half = average_gas_density_at_half *  4.18879
	
	
	#######  HEre are the radii ratios
	ratio_of_gas = half_gas_mass_radius  / half_dm_mass_radius
	
	ratio_of_stars = half_star_mass_radius / half_dm_mass_radius
	
	gas_to_dm_radius.insert(n,ratio_of_gas)
	star_to_dm_radius.insert(n,ratio_of_stars)
	
	### Here are the densities at half radii
	
	dark_density_half.insert(n,average_dark_density_at_half)
	gas_density_half .insert(n,average_gas_density_at_half )
	star_density_half.insert(n,average_star_density_at_half)
	
	total_star_mass.insert(n,star_mass)
	total_gas_mass.insert(n,gas_mass)
	total_dm_mass.insert(n,dm_mass_tot)
	
	
	
	#######  HEre are the radii ratios
	#gas_radii_ratio.insert(n,gas_radii_ratio1)
	#star_radii_ratio.insert(n,star_radii_ratio1)
	...
	filename17 = "dark_density_half.txt"
	filename18 = "star_density_half.txt"
	filename19 = "gas_density_half.txt"
	filename20 =  "star_radii_ratio.txt"
	filename21 = "gas_radii_ratio.txt"
	filename22 = "total_star_mass.txt"
	filename23 = "total_gas_mass.txt"
	filename24 = "total_dm_mass.txt"
...
data = open(filename17,"w")

for c in dark_density_half :
	data.write("%s\n" % c)
	
data.close()

data = open(filename18,"w")

for c in star_density_half :
	data.write("%s\n" % c)
	
data.close()


data = open(filename19,"w")

for c in gas_density_half :
	data.write("%s\n" % c)
	
data.close()


data = open(filename20,"w")

for c in star_to_dm_radius :
	data.write("%s\n" % c)
	
data.close()


data = open(filename21,"w")

for c in gas_to_dm_radius :
	data.write("%s\n" % c)
...
data = open(filename22,"w")

for c in total_star_mass :
	data.write("%s\n" % c)
...
data = open(filename23,"w")

for c in total_gas_mass :
	data.write("%s\n" % c)
...
data = open(filename24,"w")

for c in total_dm_mass :
	data.write("%s\n" % c)
